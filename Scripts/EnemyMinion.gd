...
const STOP_RADIUS := 60.0     # Enemy stops chasing when this close — prevents magnet lock
...
func _physics_process(_delta: float) -> void:
	...
	if dist > DETECT_RADIUS:
		# Out of detection range — stay still
		velocity = Vector2.ZERO
	elif dist > STOP_RADIUS:
		# In range but not too close — chase
		velocity = (player.global_position - global_position).normalized() * CHASE_SPEED
	else:
		# Within stop radius — halt, no pushing into player
		velocity = Vector2.ZERO
	move_and_slide()
...
func _die() -> void:
	is_dead = true
	hp = 0
	sprite.color = Color(0.3, 0.1, 0.1)
	hp_label.text = "DEAD"
...
func _respawn() -> void:
	...
	sprite.color = Color(0.8, 0.15, 0.15)
